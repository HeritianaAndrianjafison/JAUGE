...
if($action == null){
	...
	$n = DB::select("station",array("COUNT(*)"),"$condition")[0]["COUNT(*)"];
			...
			$nav="<ul class='pagination'><li><a href='jauges.php' >Tous</a></li>";
			for($i=0;$i<$q;$i++){
				$p=$i+1;
				$class = "";
				if($i==$page){
				...
				$nav.="<li $class><a href='jauges.php?page=$i&&filtre=".$filtre."&&region=$region' >$p</a></li>";
				
			}
			$nav.="</ul>";
	$region =  "<option value='".$region."' >".label_region($region)."</option>".liste_region();
	include "include/list_jauge.php";
}
if($action == "new"){
